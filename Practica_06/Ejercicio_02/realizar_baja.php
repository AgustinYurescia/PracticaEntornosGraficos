...
<body>
    <?php
        ...
        $id = $_POST['id_ciudad'];
        $sentenciaSQL = "SELECT id FROM ciudades WHERE id = '$id'";
        $rta =  mysqli_query($link, $sentenciaSQL) or die (mysqli_error($link));
        $row_encontrada =  mysqli_fetch_assoc($rta);
        if ($row_encontrada){
            $sentenciaSQL = "DELETE FROM ciudades WHERE id = '$id'";
            mysqli_query($link, $sentenciaSQL) or die (mysqli_error($link));
            echo("<div class='alert alert-success' role='alert'>¡Baja Realizada! - <a href='home.html' class='alert-link'>Volver a inicio</a></div>");
